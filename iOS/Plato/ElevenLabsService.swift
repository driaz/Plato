...
final class ElevenLabsService: NSObject, ObservableObject, AVSpeechSynthesizerDelegate, AVAudioPlayerDelegate  {
    ...
    /// Speak text using ElevenLabs TTS when available; fallback to system TTS otherwise.
    func speak(_ raw: String) async {
        stopSpeaking()
        let text = Self.cleanText(raw)

        // Always use blocking MP3 mode for stability
        do {
    ...
    // MARK: - Blocking George playback (MP3) - FIXED VERSION
    ...
    private func playMP3(_ data: Data) async {
        ...
        // Create a completion handler for when audio finishes
        await withCheckedContinuation { continuation in
            node.scheduleFile(file, at: nil) {
                // This completion handler is called when the audio file finishes playing
                continuation.resume()
            }
            node.play()
        }
        
        // Add a small delay to ensure the last bit of audio plays through the pipeline
        try? await Task.sleep(nanoseconds: 500_000_000) // 0.5 seconds safety margin
